        ...
        TOPOPIX **TopoMap, SNOWPIX **Snow, int Dt)
{
  ...
  // printf("current DNum is %d \n", DNum);
  for (y = 0; y < Map->NY; y++) {
    for (x = 0; x < Map->NX; x++) {
      if (INBASIN(TopoMap[y][x].Mask)) {
           //printf("current SWE is %f \n", Snow[y][x].Swq);
          ...
          if ( Snow[y][x].Swq > Snow[y][x].MaxSwe){
            ...
            overwrites previous incorrect dates*/
          ...
          /* Criteria :
            ...
            3. And Preceding 7/15 day has snow  // for now this was not implemented
          */
         if ((Snow[y][x].Swq < MELTOUT_SWE) && (DNum > Snow[y][x].MaxSweDate) && (Snow[y][x].MeltOutDate == 0)){
