...
BeginDate = as.Date(paste0(CalibrationWaterYears[1]-1,"-10-01"), format="%Y-%m-%d")
EndDate = as.Date(paste0(CalibrationWaterYears[nWaterYears],"-09-01"), format="%Y-%m-%d")
...
legend("topleft",inset=0.1,legend=c("Reconstructed Hydrograph",paste0(100*HighFlowQuantile,"th Percentile Threshold")),lty=c(1,2),lwd=c(2,2),col=c("black","red"),cex=1.2)
...
lines(Dates,rep(HighFlowThresh,length(Ptrs)),col="red",lwd=2,lty=2)
legend("bottomleft",inset=0.05,legend=c("Reconstructed Hydrograph","90th Percentile Threshold"),lty=c(1,2),lwd=c(2,2),col=c("black","red"),cex=1.2)
...
write.csv(PeakFlowData, paste0("YubaFNF_",100*HighFlowQuantile,"thPercentileFlows.csv"))
