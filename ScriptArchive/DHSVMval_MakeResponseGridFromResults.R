...
# Validation: post-process new runs

################################################################################
# Universal adjustable settings

ValModels = c(195, 239, 244, 270, 276, 294)
...
dir = "C:/Users/board/Documents/DHSVM/DHSVM-PNNL-master/Calibrate_GPEHRL/Validation/"
setwd(dir)
...
results.grid.val = data.frame(matrix(nrow=length(ValModels),ncol=(nObjFuns+1)))
names(results.grid.val) = c("ParamSet",ObjFunNames)
results.grid.val$ParamSet = ValModels
print(results.grid.val)
...
# NOTE also that this is how we determine which pixels get compared--this mask ONLY includes the validation watersheds!
...
for (i in ValModels){
  ...
  if (!all(dir.exists(paste0(ResultsDir,"Val_Params",i,"_",BasinNames)))){
    
    results.grid.val[results.grid.val$ParamSet==i,]$PeakSWE_RMSE = NA
    # results.grid.val[results.grid.val$ParamSet==i,]$PeakDate_RMSE = NA
    ...
    results.grid.val[results.grid.val$ParamSet==i,]$PeakSWE_RMSE = RMSEvalSWE
    
    # RMSEvalDate = sqrt(mean((DHSVMdateVals - DateReferenceVals)^2))
    # results.grid.val[results.grid.val$ParamSet==i,]$PeakDate_RMSE = RMSEvalDate
...
# For now, we keep only the exact same objective functions (add American later)
...
WaterYieldReference = read.csv(paste0(ReferenceDir,"YearlyTotalRunoff_YubaAndTruckee_Val.csv"))[,-1]
print(WaterYieldReference)

# Read pre-calculated data from Yuba FNF point, with a selected number of the highest flows
HighFlowReference = read.csv(paste0(ReferenceDir,"YubaFNF_95thPercentileFlows_Val.csv"))[,-1]
head(HighFlowReference)
HighFlowReferenceVals = rev(sort(HighFlowReference$DailyAvgQ))

# Loop over each parameter set and calculate the objective function value
for (i in results.grid.val$ParamSet){
  
  # Check whether results are available for this parameter set
  if (!all(dir.exists(paste0(ResultsDir,"Val_Params",i,"_",BasinNames)))){
    
    # results.grid.val[results.grid.val$ParamSet==i,]$Headwaters_NSE = NA
    # results.grid.val[results.grid.val$ParamSet==i,]$Downstream_NSE = NA
    results.grid.val[results.grid.val$ParamSet==i,]$Total_NSE = NA
    # results.grid.val[results.grid.val$ParamSet==i,]$Yearly_NSE = NA
    results.grid.val[results.grid.val$ParamSet==i,]$LowBase_LogNSE = NA
    results.grid.val[results.grid.val$ParamSet==i,]$HighBase_LogNSE = NA
    results.grid.val[results.grid.val$ParamSet==i,]$WaterYield_RMSE = NA
    results.grid.val[results.grid.val$ParamSet==i,]$PeakFlow_RMSE = NA
    ...
    results.grid.val[results.grid.val$ParamSet==i,]$Total_NSE = WeightedTotalNSE
    # results.grid.val[results.grid.val$ParamSet==i,]$Yearly_NSE = WeightedYearlyNSE
    results.grid.val[results.grid.val$ParamSet==i,]$LowBase_LogNSE = WeightedLowBaseLogNSE
    results.grid.val[results.grid.val$ParamSet==i,]$HighBase_LogNSE = WeightedHighBaseLogNSE
    ...
    results.grid.val[results.grid.val$ParamSet==i,]$WaterYield_RMSE = BulkRMSE
    ...
    results.grid.val[results.grid.val$ParamSet==i,]$PeakFlow_RMSE = PeakRMSE
...
#plot(results.grid.val$Total_NSE, results.grid.val$HighBase_LogNSE, ylim=c(0,1))

################################################################################
# Save the actual objective function values (not scaled)

write.csv(results.grid.val,"Validation_ResultsGrid.csv")

################################################################################
#  Just for fun, see if all validation sets are Pareto-optimal
################################################################################

library(GPareto)

# Simplify the structure into pure matrices of numbers
results.grid = unname(as.matrix(results.grid.val[,2:(1+nObjFuns)],ncol=nObjFuns))

nDesigns = length(results.grid[,1]) # For easy usage elsewhere

# NSE is natively scaled -Inf --> worst, 1 --> best
# Convert to 1-NSE so it's scaled 0 --> best, Inf --> worst
for (obj in 1:nObjFuns){
  if (grepl("NSE",names(results.grid.val)[obj+1])){
    results.grid[,obj] = 1 - results.grid[,obj]
  }
}

# Calculate current Pareto front
ParetoFront = t(nondominated_points(t(results.grid[,-1])))

length(ParetoFront[,1])
